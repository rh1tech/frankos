...
static void shell_run_elf(int argc, char **argv) {
    ...
    /* Execute — this blocks until the app finishes (attached) */
    printf("[shell] calling exec...\n");
    exec(ctx);
    printf("[shell] exec returned, stage=%d ret=%d\n", ctx->stage, ctx->ret_code);

    /* If the app prepared a follow-on command (e.g. mc launching mcview),
     * ctx->stage == PREPARED and ctx->argv/orig_cmd point to the next app.
     * Loop until no more chained commands remain. */
